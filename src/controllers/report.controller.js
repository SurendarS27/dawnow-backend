        ...
        if (!isMonthly) {
            ...
            staffList.forEach((staff, sIdx) => {
                ...
                dateRange.forEach(date => {
                    ...
                    const cleanItem = (str) => {
                        if (!str) return '';
                        // Remove leading numbers/dots/bullets (e.g. "1. " or "2)")
                        let cleaned = str.trim().replace(/^[0-9]+[\.\)\-\s]+/, '').trim();
            ...
            staffList.forEach((staff, sIdx) => {
                ...
                dateRange.forEach((date, dIdx) => {
                    ...
                    if (dayLog && dayLog.isLeaveDay) items = ['Leave'];
                        ...
                        // Include Daily Log entry
                        if (dayLog && dayLog.workDone) {
                            items.push(`${dayLog.category || 'Daily Work'}: ${dayLog.workDone.trim().substring(0, 100)}`);
                        }
